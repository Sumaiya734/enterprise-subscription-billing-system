                        ...
                        <div class="card border-0 shadow-sm h-100">
                            <div class="card-body">
                                ...
                                <small class="form-text text-muted d-block">
                                    ...
                                    Max file size: 2MB per image | Allowed formats: JPG, PNG, GIF
                                </small>
                            </div>
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    idCardFrontInput.addEventListener('change', function(e) {
        ...
        // Validate file size
        validateFileSize(file, 'id_card_front');
    });
    ...
    idCardBackInput.addEventListener('change', function(e) {
        ...
        // Validate file size
        validateFileSize(file, 'id_card_back');
    });
        ...
        if (file) {
            ...
            if (file.size > maxSize) {
                ...
                if (fieldName === 'profile_picture') {
                    profilePreview.style.display = 'none';
                    noProfilePreview.style.display = 'block';
                } else if (fieldName === 'id_card_front') {
                    frontPreviewContainer.style.display = 'none';
    ...
    function validateFileType(file, fieldName) {
        ...
        if (file && !allowedTypes.includes(file.type)) {
            alert(`File "${file.name}" is not a valid image type. Allowed types: JPG, PNG, GIF.`);
            document.getElementById(fieldName).value = '';
    ...
    // Add file type validation listeners
    profilePictureInput.addEventListener('change', function(e) {
        if (e.target.files[0]) {
            validateFileType(e.target.files[0], 'profile_picture');
        }
    });

    idCardFrontInput.addEventListener('change', function(e) {
        if (e.target.files[0]) {
            validateFileType(e.target.files[0], 'id_card_front');
        }
    });

    idCardBackInput.addEventListener('change', function(e) {
        if (e.target.files[0]) {
            validateFileType(e.target.files[0], 'id_card_back');
        }
    });
    ...
    form.addEventListener('reset', function() {
        ...
        document.getElementById('remove_id_cards')?.checked = false;
    });
});
</script>
@endsection